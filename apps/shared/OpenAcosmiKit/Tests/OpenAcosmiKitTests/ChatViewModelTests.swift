...
import Foundation
import Testing
@testable import OpenAcosmiChatUI
...
private final class TestChatTransport: @unchecked Sendable, OpenAcosmiChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenAcosmiChatHistoryPayload]
    private let sessionsResponses: [OpenAcosmiChatSessionsListResponse]

    private let stream: AsyncStream<OpenAcosmiChatTransportEvent>
    private let continuation: AsyncStream<OpenAcosmiChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenAcosmiChatHistoryPayload],
        sessionsResponses: [OpenAcosmiChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenAcosmiChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenAcosmiChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenAcosmiChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [OpenAcosmiChatAttachmentPayload]) async throws -> OpenAcosmiChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenAcosmiChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenAcosmiChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenAcosmiChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenAcosmiChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = OpenAcosmiChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = OpenAcosmiChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { OpenAcosmiChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = OpenAcosmiChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = OpenAcosmiChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = OpenAcosmiChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { OpenAcosmiChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }

...
enum OpenAcosmiChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("OpenAcosmiChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("OpenAcosmiChatTheme.onboardingAssistantBubble"),
...
enum OpenAcosmiPlatformImageFactory {
    static func image(_ image: OpenAcosmiPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)

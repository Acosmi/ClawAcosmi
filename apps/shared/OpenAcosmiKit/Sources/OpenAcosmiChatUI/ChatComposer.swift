...
@MainActor
struct OpenAcosmiChatComposer: View {
    @Bindable var viewModel: OpenAcosmiChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(OpenAcosmiChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(OpenAcosmiChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OpenAcosmiPendingAttachment.id)
                { (att: OpenAcosmiPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OpenAcosmiPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(OpenAcosmiChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenAcosmiChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(OpenAcosmiChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(OpenAcosmiChatTheme.subtleCard)
        .clipShape(Capsule())
    }

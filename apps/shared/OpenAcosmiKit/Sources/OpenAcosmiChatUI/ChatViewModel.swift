...
private let chatUILogger = Logger(subsystem: "ai.openacosmi", category: "OpenAcosmiChatUI")

@MainActor
@Observable
public final class OpenAcosmiChatViewModel {
    public private(set) var messages: [OpenAcosmiChatMessage] = []
    ...
    public var attachments: [OpenAcosmiPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenAcosmiChatPendingToolCall] = []
    public private(set) var sessions: [OpenAcosmiChatSessionEntry] = []
    private let transport: any OpenAcosmiChatTransport
    ...
    private var pendingToolCallsById: [String: OpenAcosmiChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenAcosmiChatTransport) {
    ...
    public var sessionChoices: [OpenAcosmiChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [OpenAcosmiChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [OpenAcosmiChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenAcosmiPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenAcosmiChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenAcosmiChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [OpenAcosmiChatMessage]) -> [OpenAcosmiChatMessage] {
        var result: [OpenAcosmiChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenAcosmiChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenAcosmiChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> OpenAcosmiChatAttachmentPayload in
            OpenAcosmiChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenAcosmiChatSessionEntry {
        OpenAcosmiChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenAcosmiChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenAcosmiChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenAcosmiAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenAcosmiChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenAcosmiPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)

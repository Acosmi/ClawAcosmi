...
struct ChatMessageBubble: View {
    let message: OpenAcosmiChatMessage
    let style: OpenAcosmiChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OpenAcosmiChatTheme.userText : OpenAcosmiChatTheme.assistantText
    ...
    private var inlineAttachments: [OpenAcosmiChatMessageContent] {
    ...
    private var toolCalls: [OpenAcosmiChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OpenAcosmiChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OpenAcosmiChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OpenAcosmiChatTheme.onboardingAssistantBubble
        }
        return OpenAcosmiChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OpenAcosmiChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OpenAcosmiChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OpenAcosmiChatTheme.userText : OpenAcosmiChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OpenAcosmiChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(OpenAcosmiChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(OpenAcosmiChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OpenAcosmiChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OpenAcosmiChatTheme.assistantText)
            }
        }
    }
}

...
This is an **alpha** iOS app that connects to an OpenAcosmi Gateway as a `role: node`.
...
3. On iOS: OpenAcosmi → Settings → Gateway → paste setup code → Connect
...
1. Select the `OpenAcosmi` scheme
...
cd apps/ios
xcodegen generate
xcodebuild test -project OpenAcosmi.xcodeproj -scheme OpenAcosmi -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/OpenAcosmiKit` contains the shared transport/types used by the iOS app.

...
@Suite struct GatewayEndpointIDTests {
    @Test func stableIDForServiceDecodesAndNormalizesName() {
        let endpoint = NWEndpoint.service(
            name: "OpenAcosmi\\032Gateway   \\032  Node\n",
            type: "_openacosmi-gw._tcp",
            domain: "local.",
            interface: nil)

        #expect(GatewayEndpointID.stableID(endpoint) == "_openacosmi-gw._tcp|local.|OpenAcosmi Gateway Node")
    }

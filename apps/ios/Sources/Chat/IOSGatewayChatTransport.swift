...
struct IOSGatewayChatTransport: OpenAcosmiChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> OpenAcosmiChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenAcosmiChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenAcosmiChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenAcosmiChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenAcosmiChatAttachmentPayload]) async throws -> OpenAcosmiChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenAcosmiChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(OpenAcosmiChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(OpenAcosmiGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenAcosmiGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenAcosmiChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenAcosmiAgentEventPayload.self)
                        {

...
final class MotionService: MotionServicing {
    func activities(params: OpenAcosmiMotionActivityParams) async throws -> OpenAcosmiMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenAcosmiMotionActivityEntry], Error>) in
        ...
        return OpenAcosmiMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenAcosmiPedometerParams) async throws -> OpenAcosmiPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<OpenAcosmiPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenAcosmiPedometerPayload(

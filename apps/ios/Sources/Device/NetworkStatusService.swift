...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> OpenAcosmiNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> OpenAcosmiNetworkStatusPayload {
        let status: OpenAcosmiNetworkPathStatus = switch path.status {
        ...
        var interfaces: [OpenAcosmiNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return OpenAcosmiNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> OpenAcosmiNetworkStatusPayload {
        OpenAcosmiNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])

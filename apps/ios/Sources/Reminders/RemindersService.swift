...
final class RemindersService: RemindersServicing {
    func list(params: OpenAcosmiRemindersListParams) async throws -> OpenAcosmiRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[OpenAcosmiReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenAcosmiReminderPayload(
        ...
        return OpenAcosmiRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenAcosmiRemindersAddParams) async throws -> OpenAcosmiRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenAcosmiReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenAcosmiRemindersAddPayload(reminder: payload)
    }

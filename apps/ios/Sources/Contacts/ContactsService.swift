...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenAcosmiContactsSearchParams) async throws -> OpenAcosmiContactsSearchPayload {
        ...
        return OpenAcosmiContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenAcosmiContactsAddParams) async throws -> OpenAcosmiContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenAcosmiContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenAcosmiContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenAcosmiContactPayload {
        OpenAcosmiContactPayload(

...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openacosmi", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenAcosmiSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenAcosmiSessionPreviewEntry,
    ...
    private static func cache(payload: OpenAcosmiSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenAcosmiChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenAcosmiKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenAcosmiKit.AnyCodable]) -> [OpenAcosmiChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenAcosmiChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenAcosmiChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenAcosmiChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenAcosmiChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenAcosmiChatMessage) -> String? {

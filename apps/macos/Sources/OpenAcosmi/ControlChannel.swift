...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenAcosmiProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openacosmi", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenAcosmiKit.AnyCodable]()) {
                $0[$1.key] = OpenAcosmiKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenAcosmiProtocol.AnyCodable?) -> [String: OpenAcosmiProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenAcosmiProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenAcosmiKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenAcosmiProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openacosmi.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openacosmi.control.agent")
}

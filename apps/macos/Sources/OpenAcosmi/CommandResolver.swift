...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openacosmi.gatewayProjectRootPath"
    private static let helperName = "openacosmi"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openacosmiEntry = root.appendingPathComponent("openacosmi.mjs").path
        if FileManager().isReadableFile(atPath: openacosmiEntry) { return openacosmiEntry }
        let binEntry = root.appendingPathComponent("bin/openacosmi.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openacosmi")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openacosmiPaths = self.openacosmiManagedPaths(home: home)
        if !openacosmiPaths.isEmpty {
            extras.insert(contentsOf: openacosmiPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openacosmiPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openacosmiManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openacosmi"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openacosmiExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenAcosmiExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openacosmi.mjs").path,
            root.appendingPathComponent("bin/openacosmi.js").path,
        ]
    ...
    static func hasAnyOpenAcosmiInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openacosmiExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openacosmiNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let openacosmiPath = self.projectOpenAcosmiExecutable(projectRoot: root) {
                return [openacosmiPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "openacosmi", subcommand] + extraArgs
            }
            if let openacosmiPath = self.openacosmiExecutable(searchPaths: searchPaths) {
                return [openacosmiPath, subcommand] + extraArgs
            }

            let missingEntry = """
            openacosmi entrypoint missing (looked for dist/index.js or openacosmi.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openacosmi CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openacosmi"
        ...
        \(cliSection)
        \(projectSection)
        if command -v openacosmi >/dev/null 2>&1; then
          CLI="$(command -v openacosmi)"
          openacosmi \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openacosmi.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openacosmi.mjs"
            node "$PRJ/openacosmi.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openacosmi.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openacosmi.js"
            node "$PRJ/bin/openacosmi.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openacosmi"
          pnpm --silent openacosmi \(quotedArgs);
        else
          echo "openacosmi CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenAcosmiConfigFile.loadDict()

...
private let deepLinkLogger = Logger(subsystem: "ai.openacosmi", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "OpenAcosmi is paused", message: "Unpause OpenAcosmi to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run OpenAcosmi agent?", message: body) else { return }
        }

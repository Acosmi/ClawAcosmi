...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openacosmi/openacosmi.json. Prefer OPENACOSMI_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openacosmi/openacosmi.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openacosmi/openacosmi.json. Restart the gateway to apply."
        }

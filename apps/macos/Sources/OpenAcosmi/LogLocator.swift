...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENACOSMI_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/openacosmi")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openacosmi-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openacosmi-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openacosmi/ (rolling or stdout), or nil if none exist.

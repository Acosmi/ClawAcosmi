...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.openacosmi", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: OpenAcosmiPeekabooBridgeServices?
    private static var openacosmiSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("OpenAcosmi", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = OpenAcosmiPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.openacosmiSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.openacosmiSocketPath, privacy: .public)")
    }
...
@MainActor
private final class OpenAcosmiPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.openacosmi.peekaboo")

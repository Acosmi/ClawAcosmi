...
enum OpenAcosmiLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenAcosmiOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenAcosmiFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenAcosmiLogging.bootstrapIfNeeded()
        let label = OpenAcosmiLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenAcosmiOSLogHandler: LogHandler {
...
struct OpenAcosmiFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenAcosmiLogging.parseLabel(self.label)

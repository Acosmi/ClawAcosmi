...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openacosmi.mac`) even if the shipped
    ...
    static func resolveNixMode(
        ...
        if environment["OPENACOSMI_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openacosmi.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openacosmi.nixMode") { return true }

...
enum DebugActions {
    private static let verboseDefaultsKey = "openacosmi.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openacosmi.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenAcosmiPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenAcosmi", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenAcosmiPaths.configURL
        guard

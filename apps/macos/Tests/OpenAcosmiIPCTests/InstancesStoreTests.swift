...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenAcosmiProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenAcosmiProtocol.AnyCodable] = [
            "presence": .init([OpenAcosmiProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenAcosmiProtocol.AnyCodable(payloadMap)

...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: OpenAcosmiProtocol.AnyCodable(["ok": OpenAcosmiProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: OpenAcosmiProtocol.AnyCodable(["ok": OpenAcosmiProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = OpenAcosmiProtocol.AnyCodable([
            "runId": OpenAcosmiProtocol.AnyCodable("run-1"),
            "sessionKey": OpenAcosmiProtocol.AnyCodable("main"),
            "state": OpenAcosmiProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: OpenAcosmiProtocol.AnyCodable(["a": OpenAcosmiProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)

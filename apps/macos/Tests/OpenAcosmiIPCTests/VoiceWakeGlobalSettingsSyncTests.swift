...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = OpenAcosmiProtocol.AnyCodable(["triggers": ["openacosmi", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["openacosmi", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = OpenAcosmiProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,

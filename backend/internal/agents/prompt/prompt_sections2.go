...
func buildMessagingSection(isMinimal bool, available map[string]bool, messageToolHints []string) string {
	...
	lines := []string{
		...
		"- Never use exec/curl for provider messaging; OpenAcosmi handles all routing internally.",
	}
...
func buildDocsSection(docsPath string, isMinimal bool) string {
	...
	return fmt.Sprintf("## Documentation\n"+
		"OpenAcosmi docs: %s\n"+
		"Mirror: https://docs.openacosmi.ai\n"+
		"Source: https://github.com/openacosmi/openacosmi\n"+
...
func buildHeartbeatsSection(heartbeatPrompt string) string {
	...
	return "## Heartbeats\n" + line + "\n" +
		...
		"HEARTBEAT_OK\n" +
		"OpenAcosmi treats a leading/trailing \"HEARTBEAT_OK\" as a heartbeat ack (and may discard it).\n" +

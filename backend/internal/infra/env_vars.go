...
func AllowMultiGateway() bool {
	return isTruthyEnvValue(os.Getenv("OPENACOSMI_ALLOW_MULTI_GATEWAY"))
}
...
func SessionManagerCacheTTLMs() int {
	raw := strings.TrimSpace(os.Getenv("OPENACOSMI_SESSION_MANAGER_CACHE_TTL_MS"))
	if raw == "" {
		return -1
	}
	parsed, err := strconv.Atoi(raw)
...
func UpdateInProgress() bool {
	return isTruthyEnvValue(os.Getenv("OPENACOSMI_UPDATE_IN_PROGRESS"))
}
...
func TestHandshakeTimeoutMs() int {
	raw := strings.TrimSpace(os.Getenv("OPENACOSMI_TEST_HANDSHAKE_TIMEOUT_MS"))
	if raw == "" {
		return 0
	}
	parsed, err := strconv.Atoi(raw)
...
func NoRespawn() bool {
	return isTruthyEnvValue(os.Getenv("OPENACOSMI_NO_RESPAWN"))
}
...
func NodeExecHostMode() string {
	return strings.TrimSpace(strings.ToLower(os.Getenv("OPENACOSMI_NODE_EXEC_HOST")))
}
...
func NodeExecFallbackAllowed() bool {
	raw := strings.TrimSpace(strings.ToLower(os.Getenv("OPENACOSMI_NODE_EXEC_FALLBACK")))
	return raw != "0"
}
...
func BrowserEnabled() bool {
	raw := strings.TrimSpace(os.Getenv("OPENACOSMI_BROWSER_ENABLED"))
	if raw == "" {
		return true // 默认启用
	}

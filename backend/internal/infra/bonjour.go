...
func isBonjourDisabledByEnv() bool {
	v := os.Getenv("OPENACOSMI_DISABLE_BONJOUR")
...
func prettifyInstanceName(name string) string {
	...
	re := regexp.MustCompile(`(?i)\s+\(OpenAcosmi\)\s*$`)
	result := strings.TrimSpace(re.ReplaceAllString(normalized, ""))
	if result == "" {
		return normalized
	}
	return result
...
func resolveHostname() string {
	if v := strings.TrimSpace(os.Getenv("OPENACOSMI_MDNS_HOSTNAME")); v != "" {
...
func BuildBonjourTXTRecords(opts GatewayBonjourAdvertiseOpts) map[string]string {
	...
	if strings.TrimSpace(instanceName) == "" {
		instanceName = fmt.Sprintf("%s (OpenAcosmi)", hostname)
	}

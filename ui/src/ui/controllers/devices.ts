...
import { showConfirmDialog, showPromptDialog } from "../components/confirm-dialog.ts";
...
export async function rejectDevicePairing(state: DevicesState, requestId: string) {
  ...
  const confirmed = await showConfirmDialog(t("devices.confirmReject"));
  if (!confirmed) {
    return;
  }
  try {
...
export async function rotateDeviceToken(
  ...
  try {
    ...
    if (res?.token) {
      ...
      await showPromptDialog(t("devices.newTokenPrompt"), res.token);
    }
    await loadDevices(state);
  } catch (err) {
    state.devicesError = String(err);
...
export async function revokeDeviceToken(
  ...
  const confirmed = await showConfirmDialog(
    t("devices.confirmRevoke", { deviceId: params.deviceId, role: params.role }),
  );
  if (!confirmed) {
    return;
  }

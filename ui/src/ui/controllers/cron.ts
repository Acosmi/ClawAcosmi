...
async function refreshCronData(state: CronState) {
  await Promise.all([loadCronJobs(state), loadCronStatus(state)]);
}
...
export async function runCronJob(state: CronState, job: CronJob) {
  ...
  void loadCronRuns(state, job.id);
}

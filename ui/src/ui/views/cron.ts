...
function resolveChannelLabel(props: CronProps, channel: string): string {
  ...
  const meta = Array.isArray(props.channelMeta) ? props.channelMeta.find((entry) => entry.id === channel) : undefined;
  if (meta?.label) {
    return meta.label;
  }
...
export function renderCron(props: CronProps) {
  ...
  const selectedRunTitle = selectedJob?.name ?? props.runsJobId ?? t("cron.selectJobHint");
  ...
  return html`
        ...
        <div class="stat-grid" style="margin-top: 16px;">
          <div class="stat">
            ...
            <div class="stat-value">
              ${props.status ? (props.status.enabled ? t("channels.yes") : t("channels.no")) : "n/a"}
            </div>
          </div>
          <div class="stat">
      ...
      props.onFormChange({
        wakeMode: (e.target as HTMLSelectElement).value as CronFormState["wakeMode"],
            ...
            >
              ...
              <option value="next-heartbeat">${t("cron.wakeModeHeartbeat")}</option>
            </select>
          </label>
          <label class="field">
      ...
      props.onFormChange({
        payloadKind: (e.target as HTMLSelectElement)
          .value as CronFormState["payloadKind"],
            ...
            >
              <option value="systemEvent">${t("cron.payloadSystem")}</option>
              <option value="agentTurn">${t("cron.payloadAgent")}</option>
            </select>
          </label>
        </div>
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">${t("cron.runHistorySub", { name: selectedRunTitle })}</div>
      ...
      <div class="cron-job-footer">
        <div class="chip-row cron-job-chips">
          <span class=${`chip ${job.enabled ? "chip-ok" : "chip-danger"}`}>
            ${job.enabled ? t("cron.enabled") : t("agents.skill.disabled")}
          </span>

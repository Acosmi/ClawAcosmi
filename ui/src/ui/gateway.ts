...
// ---------------------------------------------------------------------------
// Inlined from src/gateway/protocol/client-info.ts (UI-used subset only)
// ---------------------------------------------------------------------------

const GATEWAY_CLIENT_NAMES = {
  CONTROL_UI: "openacosmi-control-ui",
} as const;
...
export class GatewayBrowserClient {
  ...
  private connect() {
    ...
    // Ensure the URL includes the /ws path (backend registers WS handler at /ws)
  ...
  private async sendConnect() {
    ...
    // Send standard protocol frame: {type:"req", id, method:"connect", params}
    const connectId = generateUUID();
    const connectFrame = { type: "req" as const, id: connectId, method: "connect" as const, params };
    const connectPromise = new Promise<GatewayHelloOk>((resolve, reject) => {
      this.pending.set(connectId, { resolve: (v) => resolve(v as GatewayHelloOk), reject });
    });
  ...
  private handleMessage(raw: string) {
    ...
    if (frame.type === "res") {
      ...
      if (res.ok) {
        // The connect response wraps hello-ok in the payload

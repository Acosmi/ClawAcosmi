...
declare global {
  interface Window {
    __OPENACOSMI_ASSISTANT_NAME__?: string;
    __OPENACOSMI_ASSISTANT_AVATAR__?: string;
  }
}
...
export function resolveInjectedAssistantIdentity(): AssistantIdentity {
  ...
  return normalizeAssistantIdentity({
    name: window.__OPENACOSMI_ASSISTANT_NAME__,
    avatar: window.__OPENACOSMI_ASSISTANT_AVATAR__,
  });
}

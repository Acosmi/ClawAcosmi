...
OpenAcosmi supports **camera capture** for agent workflows:
...
```bash
openacosmi nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
...
- **Settings → General → Allow Camera** (`openacosmi.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `openacosmi` CLI to invoke camera commands on the macOS node.

Examples:

```bash
openacosmi nodes camera list --node <id>            # list camera ids
openacosmi nodes camera snap --node <id>            # prints MEDIA:<path>
...
openacosmi nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
openacosmi nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
...
- `openacosmi nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
openacosmi nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```

...
macOS can also run in **node mode**: the menubar app connects to the Gateway’s WS server and exposes its local canvas/camera commands as a node (so `openacosmi nodes …` works against this Mac).
...
- `node.pair.*` (CLI: `openacosmi nodes pending/approve/reject`) is a separate gateway-owned
...
- **Approvals**: enforced on the node host via `~/.openacosmi/exec-approvals.json`.
...
- The token is `gateway.auth.token` from the gateway config (`~/.openacosmi/openacosmi.json` on the gateway host).
- `openacosmi node run` reads `OPENACOSMI_GATEWAY_TOKEN` for auth.
...
- `--display-name` on `openacosmi node run` / `openacosmi node install` (persists in `~/.openacosmi/node.json` on the node).
- `openacosmi nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
Approvals live on the node host at `~/.openacosmi/exec-approvals.json`.
...
openacosmi nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
...
- On headless node host, `system.run` is gated by exec approvals (`~/.openacosmi/exec-approvals.json`).
...
OpenAcosmi can run a **headless node host** (no UI) that connects to the Gateway
...
- The node host stores its node id, token, display name, and gateway connection info in `~/.openacosmi/node.json`.
- Exec approvals are enforced locally via `~/.openacosmi/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host prefers the companion app exec host when reachable and falls
  back to local execution if the app is unavailable. Set `OPENACOSMI_NODE_EXEC_HOST=app` to require
  the app, or `OPENACOSMI_NODE_EXEC_FALLBACK=0` to disable fallback.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `openacosmi nodes …` works against this Mac).

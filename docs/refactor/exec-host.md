...
>
> - Exec 工具实现：`backend/internal/agents/bash/exec_tool.go`（已支持 `exec.host` 路由）
> - 节点命令策略：`backend/internal/gateway/node_command_policy.go`
> - 节点运行器：`backend/internal/nodehost/runner.go`
> - 配置 schema：`backend/internal/config/schema_hints_data.go`（`tools.exec.host`）
...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.openacosmi/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.openacosmi/node.json` (node id + pairing token).
...
Path: `~/.openacosmi/exec-approvals.json`
...
- Unix socket at `~/.openacosmi/exec-approvals.sock` (0600).

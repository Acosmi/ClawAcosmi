...
>
> - Gateway WebSocket 服务：`backend/internal/gateway/ws_server.go`（默认端口 `19001`）
> - Bridge 服务：`backend/bridge/`（默认端口 `18790`）
> - 节点命令策略：`backend/internal/gateway/node_command_policy.go`
> - 节点宿主运行器：`backend/internal/nodehost/runner.go`
...
- Code:
  ...
  - `backend/internal/gateway/ws_client.go`
...
- Code:
  - `backend/bridge/`
  - `backend/internal/gateway/server_bridge.go`
  - `backend/internal/nodehost/runner.go`
  - `docs/gateway/bridge-protocol.md`

## Control plane clients today

- CLI → Gateway WS via `callGateway`（`cli-rust/crates/oa-rpc/`）。
...
- `backend/bridge/tls/`
- fingerprint verification logic in `backend/internal/nodehost/runner.go`

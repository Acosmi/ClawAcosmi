---
summary: "计划：一套干净的插件 SDK + 运行时，用于所有消息连接器"
read_when:
  - 定义或重构插件架构
  - 将通道连接器迁移到插件 SDK/运行时
title: "插件 SDK 重构"
...
>
> - 插件注册在 **Go Gateway**（`backend/internal/gateway/plugin_registry.go`）
> - 插件管理命令由 **Rust CLI** 实现（`cli-rust/crates/oa-cmd-plugins/`）
> - 通道适配器在 `backend/internal/channels/`
...
- Publish as `openacosmi/plugin-sdk` (or export from core under `openacosmi/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenAcosmiPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenAcosmiConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenAcosmiConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenAcosmiConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenAcosmiConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenAcosmiConfig): string;
  };
};
```
...
- Introduce `openacosmi/plugin-sdk`.
- Add `api.runtime` to `OpenAcosmiPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openacosmiRuntime: ">=2026.2.0"`).

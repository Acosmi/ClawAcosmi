---
summary: "审计哪些功能会花钱、使用了哪些密钥、如何查看用量"
read_when:
  - 了解哪些功能可能调用付费 API
  - 审计密钥、成本和用量可见性
  - 解释 /status 或 /usage 成本报告
title: "API 用量与成本"
status: active
...
OpenAcosmi features that can generate provider usage or paid API calls.
...
- `openacosmi status --usage` and `openacosmi channels list` show provider **usage windows**
...
OpenAcosmi can pick up credentials from:
...
`openacosmi models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

...
OpenAcosmi integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenAcosmi owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenAcosmi spawns `imsg rpc` as a child process (legacy iMessage integration).

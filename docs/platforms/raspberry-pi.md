---
summary: "OpenAcosmi on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenAcosmi on a Raspberry Pi
  - Running OpenAcosmi on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenAcosmi on Raspberry Pi

## Goal

Run a persistent, always-on OpenAcosmi Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most OpenAcosmi features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openacosmi

# Enable if not
sudo systemctl enable openacosmi

# Start on boot
sudo systemctl start openacosmi
```
...
```bash
# Check logs
journalctl -u openacosmi --no-pager -n 100

# Common fix: rebuild
cd ~/openacosmi  # if using hackable install
npm run build
sudo systemctl restart openacosmi
```

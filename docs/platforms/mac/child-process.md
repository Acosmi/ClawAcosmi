...
service via the external `openacosmi` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`OPENACOSMI_PROFILE`; legacy `com.openacosmi.*` is supported).
...
- Writes `~/.openacosmi/disable-launchagent`.
...
`--attach-only` (or `--no-launchd`). This sets `~/.openacosmi/disable-launchagent`,

---
summary: "OpenAcosmi on Oracle Cloud (Always Free ARM)"
read_when:
  - Setting up OpenAcosmi on Oracle Cloud
  - Looking for low-cost VPS hosting for OpenAcosmi
  - Want 24/7 OpenAcosmi on a small server
title: "Oracle Cloud"
---

# OpenAcosmi on Oracle Cloud (OCI)

## Goal

Run a persistent OpenAcosmi Gateway on Oracle Cloud's **Always Free** ARM tier.

Oracle’s free tier can be a great fit for OpenAcosmi (especially if you already have an OCI account), but it comes with tradeoffs:
...
This enables Tailscale SSH, so you can connect via `ssh openacosmi` from any device on your tailnet — no public IP needed.
...
**From now on, connect via Tailscale:** `ssh ubuntu@openacosmi` (or use the Tailscale IP).

## 5) Install OpenAcosmi
...
This setup often removes the _need_ for extra host-based firewall rules purely to stop Internet-wide SSH brute force — but you should still keep the OS updated, run `openacosmi security audit`, and verify you aren’t accidentally listening on public interfaces.
...
- **Credential permissions:** `chmod 700 ~/.openacosmi`
- **Security audit:** `openacosmi security audit`
...
- `~/.openacosmi/` — config, credentials, session data
- `~/.openacosmi/workspace/` — workspace (SOUL.md, memory, artifacts)

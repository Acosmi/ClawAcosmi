...
- Jobs persist under `~/.openacosmi/cron/` so restarts don’t lose schedules.
...
openacosmi cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.openacosmi/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `openacosmi cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, OpenAcosmi defaults to `announce`.
...
- Job store: `~/.openacosmi/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.openacosmi/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
openacosmi cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
...
```bash
openacosmi system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`openacosmi system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `OPENACOSMI_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- OpenAcosmi applies exponential retry backoff for recurring jobs after consecutive errors:

...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenAcosmi
...
Running OpenAcosmi via Node with `tsx` fails at startup with:

```
[openacosmi] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openacosmi status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openacosmi.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openacosmi.mjs status` works on Node 25.

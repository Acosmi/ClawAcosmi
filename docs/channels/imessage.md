---
summary: "旧版 iMessage 支持：通过 imsg 集成（JSON-RPC over stdio）。新设置请用 BlueBubbles。"
read_when:
  - 设置 iMessage 支持
  - 调试 iMessage 发送/接收
title: "iMessage"
...
> [!IMPORTANT]
> **架构状态**：iMessage 频道由 **Go Gateway**（`backend/internal/channels/imessage/`）实现。
> CLI 命令通过 **Rust CLI** 提供。

> **推荐：** 新 iMessage 设置请使用 [BlueBubbles](/channels/bluebubbles)。
>
> `imsg` 频道是旧版外部 CLI 集成，可能在未来版本中移除。

状态：旧版外部 CLI 集成。Gateway 启动 `imsg rpc`（JSON-RPC over stdio）。
...
3. Configure OpenAcosmi with `channels.imessage.cliPath` and `channels.imessage.dbPath`.
...
- Full Disk Access for OpenAcosmi + `imsg` (Messages DB access).
...
- **Full Disk Access**: allow access for the process running OpenAcosmi (and any shell/SSH wrapper that executes `imsg`). This is required to read the Messages database (`chat.db`).
- **Automation → Messages**: allow the process running OpenAcosmi (and/or your terminal) to control **Messages.app** for outbound sends.
- **`imsg` CLI health**: verify `imsg` is installed and supports RPC (`imsg rpc --help`).

Tip: If OpenAcosmi is running headless (LaunchAgent/systemd/SSH) the macOS prompt can be easy to miss. Run a one-time interactive command in a GUI terminal to force the prompt, then retry:
...
2. Create a macOS user (example: `openacosmihome`) and sign into it.
...
If you want iMessage on another Mac, set `channels.imessage.cliPath` to a wrapper that runs `imsg` on the remote macOS host over SSH. OpenAcosmi only needs stdio.
...
**Remote attachments:** When `cliPath` points to a remote host via SSH, attachment paths in the Messages database reference files on the remote machine. OpenAcosmi can automatically fetch these over SCP by setting `channels.imessage.remoteHost`:
...
If `remoteHost` is not set, OpenAcosmi attempts to auto-detect it by parsing the SSH command in your wrapper script. Explicit configuration is recommended for reliability.
...
│ - openacosmi gateway           │          SCP (attachments)        │ - Messages signed in     │
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.openacosmi/scripts/imsg-ssh",
...
Example wrapper (`~/.openacosmi/scripts/imsg-ssh`):
...
Multi-account support: use `channels.imessage.accounts` with per-account config and optional `name`. See [`gateway/configuration`](/gateway/configuration#telegramaccounts--discordaccounts--slackaccounts--signalaccounts--imessageaccounts) for the shared pattern. Don't commit `~/.openacosmi/openacosmi.json` (it often contains tokens).
...
- Approve via:
  - `openacosmi pairing list imessage`
  - `openacosmi pairing approve imessage <CODE>`
...
If you explicitly configure a `chat_id` under `channels.imessage.groups`, OpenAcosmi treats that thread as a “group” for:

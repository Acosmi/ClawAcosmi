---
summary: "Nextcloud Talk 频道：能力、配置与运维"
read_when:
...
> [!IMPORTANT]
> **架构状态**：Nextcloud Talk 频道由 **Go Gateway**（`backend/internal/channels/nextcloud-talk/`）实现。
> CLI 命令通过 **Rust CLI** 提供。无需单独安装插件。

状态：Go Gateway 原生支持（Webhook Bot）。支持 DM、房间、反应和 Markdown 消息。

## Go Gateway 内置支持

Nextcloud Talk 频道已在 Go Gateway 中原生实现，无需单独安装插件。确保 Go Gateway 已编译并运行即可。
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenAcosmi" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openacosmi pairing list nextcloud-talk`
  - `openacosmi pairing approve nextcloud-talk <CODE>`

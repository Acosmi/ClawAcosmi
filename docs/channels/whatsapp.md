---
summary: "WhatsApp 频道集成：登录、收件箱、回复、媒体与运维"
read_when:
  - 处理 WhatsApp 频道行为或收件箱路由
title: "WhatsApp"
---
...
> [!IMPORTANT]
> **架构状态**：WhatsApp 频道由 **Go Gateway**（`backend/internal/channels/whatsapp/`）实现。
> CLI 命令通过 **Rust CLI** 提供。原始 Node.js Baileys 实现已废弃。
...
2. Configure WhatsApp in `~/.openacosmi/openacosmi.json`.
3. Run `openacosmi channels login` to scan the QR code (Linked Devices).
4. Start the gateway.
...
- **Go Gateway** 拥有 WhatsApp 连接和收件箱循环（`backend/internal/channels/whatsapp/`）。
- **Rust CLI / macOS 应用** 与 Gateway 通信；不直接操作 WhatsApp 连接。
- **活跃监听器** 是出站发送的前提；否则发送快速失败。

## Getting a phone number (two modes)

WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run OpenAcosmi on WhatsApp:

### Dedicated number (recommended)

Use a **separate phone number** for OpenAcosmi. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‑Fi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate — install WhatsApp Business and register the OpenAcosmi number there.
...
Quick fallback: run OpenAcosmi on **your own number**. Message yourself (WhatsApp “Message yourself”) for testing so you don’t spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[openacosmi]`)
...
- Early OpenAcosmi builds supported Twilio’s WhatsApp Business integration.
...
- Login command: `openacosmi channels login` (QR via Linked Devices).
- Multi-account login: `openacosmi channels login --account <id>` (`<id>` = `accountId`).
- Default account (when `--account` is omitted): `default` if present, otherwise the first configured account id (sorted).
- Credentials stored in `~/.openacosmi/credentials/whatsapp/<accountId>/creds.json`.
- Backup copy at `creds.json.bak` (restored on corruption).
- Legacy compatibility: older installs stored Baileys files directly in `~/.openacosmi/credentials/`.
- Logout: `openacosmi channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `openacosmi pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run OpenAcosmi on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will OpenAcosmi message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. OpenAcosmi only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:

- First DM from a new sender returns a short code (message is not processed).
- Approve with: `openacosmi pairing approve whatsapp <code>` (list with `openacosmi pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different OpenAcosmi instances on one WhatsApp number?**  
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `openacosmi message send --media <mp4> --gif-playback`
...
- Best results: OGG/Opus. OpenAcosmi rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/openacosmi/openacosmi-YYYY-MM-DD.log` (configurable).
...
- Fix: run `openacosmi channels login` on the gateway host and scan the QR (WhatsApp → Settings → Linked Devices).

**Linked but disconnected / reconnect loop**

- Symptom: `channels status` shows `running, disconnected` or warns “Linked but disconnected”.
- Fix: `openacosmi doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `openacosmi logs --follow`.

**Go Gateway 运行时**

- WhatsApp 频道已完全迁移至 Go Gateway，无需 Node.js 运行时。
- 使用 `make gateway-dev` 或编译后的二进制文件运行。

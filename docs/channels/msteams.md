---
summary: "Microsoft Teams 频道：能力、配置与运维"
read_when:
  - 处理 MS Teams 频道功能
...
> [!IMPORTANT]
> **架构状态**：MS Teams 频道由 **Go Gateway**（`backend/internal/channels/msteams/`）实现。
...
MS Teams 频道已在 Go Gateway 中原生实现，无需单独安装插件。确保 Go Gateway 已编译并运行即可。

## Quick setup (beginner)

1. Install the Microsoft Teams plugin.
2. Create an **Azure Bot** (App ID + client secret + tenant ID).
3. Configure OpenAcosmi with those credentials.
...
- Talk to OpenAcosmi via Teams DMs, group chats, or channels.
...
- On startup, OpenAcosmi resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.openacosmi/openacosmi.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenAcosmi, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `openacosmi-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openacosmi plugins install @openacosmi/msteams`
   - From a local checkout: `openacosmi plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenAcosmi in Teams", "full": "OpenAcosmi in Teams" },
...
OpenAcosmi handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenAcosmi only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenAcosmiShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OpenAcosmi sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openacosmi message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.openacosmi/msteams-polls.json`.
...
```bash
openacosmi message send --channel msteams \
...
openacosmi message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openacosmi message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openacosmi message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openacosmi message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```

...
Matrix is an open, decentralized messaging protocol. OpenAcosmi connects as a Matrix **user**
...
```bash
openacosmi plugins install @openacosmi/matrix
```

Local checkout (when running from a git repo):

```bash
openacosmi plugins install ./extensions/matrix
```

...
read_when:
  - You want OpenAcosmi to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables OpenAcosmi to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openacosmi onboard`) and `openacosmi channels add` list optional channel plugins.
...
```bash
openacosmi plugins install @openacosmi/nostr
```

Use a local checkout (dev workflows):

```bash
openacosmi plugins install --link <path-to-openacosmi>/extensions/nostr
```

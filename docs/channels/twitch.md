---
summary: "Twitch 频道：聊天机器人配置与设置"
read_when:
  - 为 OpenAcosmi 设置 Twitch 聊天集成
title: "Twitch"
---
...
> [!IMPORTANT]
> **架构状态**：Twitch 频道由 **Go Gateway**（`backend/internal/channels/twitch/`）实现。
> CLI 命令通过 **Rust CLI** 提供。无需单独安装插件。

Twitch 聊天支持，通过 IRC 连接。OpenAcosmi 以 Twitch 用户（Bot 账户）身份连接以接收和发送频道中的消息。

## Go Gateway 内置支持

Twitch 频道已在 Go Gateway 中原生实现，无需单独安装插件。确保 Go Gateway 已编译并运行即可。

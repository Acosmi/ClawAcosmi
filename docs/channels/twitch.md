...
read_when:
  - Setting up Twitch chat integration for OpenAcosmi
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenAcosmi connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openacosmi plugins install @openacosmi/twitch
```

Local checkout (when running from a git repo):

```bash
openacosmi plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENACOSMI_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openacosmi", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENACOSMI_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENACOSMI_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

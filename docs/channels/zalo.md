---
summary: "Zalo Bot API 频道：能力、配置与运维"
read_when:
  - 处理 Zalo 功能或 Webhook
title: "Zalo"
---
...
> [!IMPORTANT]
> **架构状态**：Zalo 频道由 **Go Gateway**（`backend/internal/channels/zalo/`）实现。
> CLI 命令通过 **Rust CLI** 提供。无需单独安装插件。

状态：实验性。仅支持 DM；群组功能即将推出。

## Go Gateway 内置支持

Zalo 频道已在 Go Gateway 中原生实现，无需单独安装插件。确保 Go Gateway 已编译并运行即可。

## Quick setup (beginner)

1. Install the Zalo plugin:
   - From a source checkout: `openacosmi plugins install ./extensions/zalo`
   - From npm (if published): `openacosmi plugins install @openacosmi/zalo`
...
- Approve via:
  - `openacosmi pairing list zalo`
  - `openacosmi pairing approve zalo <CODE>`
...
- Example: `openacosmi message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `openacosmi channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `openacosmi logs --follow`

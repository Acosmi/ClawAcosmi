...
> [!IMPORTANT]
> **架构状态**：飞书频道由 **Go Gateway**（`backend/internal/channels/feishu/`）原生实现。
> CLI 命令通过 **Rust CLI** 提供。无需单独安装插件。

Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenAcosmi to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Go Gateway 内置支持

飞书频道已在 Go Gateway 中原生实现，无需单独安装插件。确保 Go Gateway 已编译并运行即可。
...
If you just installed OpenAcosmi, run the wizard:
...
2. Configuring app credentials in OpenAcosmi
...
1. You already ran `openacosmi channels add` for Feishu
2. The gateway is running (`openacosmi gateway status`)
...
Edit `~/.openacosmi/openacosmi.json`:
...
```bash
openacosmi pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  openacosmi pairing list feishu
  openacosmi pairing approve feishu <CODE>
  ```
...
2. Run `openacosmi logs --follow` and look for `chat_id`
...
2. Run `openacosmi logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openacosmi gateway status`  | Show gateway status           |
| `openacosmi gateway install` | Install/start gateway service |
| `openacosmi gateway stop`    | Stop gateway service          |
| `openacosmi gateway restart` | Restart gateway service       |
| `openacosmi logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openacosmi logs --follow`
...
5. Ensure the gateway is running: `openacosmi gateway status`
6. Check logs: `openacosmi logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openacosmi/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openacosmi/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [

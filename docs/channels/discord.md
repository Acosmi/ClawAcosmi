---
summary: "Discord Bot API 频道：能力、配置与运维"
read_when:
  - 处理 Discord 频道功能
title: "Discord"
---
...
> [!IMPORTANT]
> **架构状态**：Discord 频道由 **Go Gateway**（`backend/internal/channels/discord/`）实现。
> CLI 命令通过 **Rust CLI** 提供。

状态：支持 DM 和 Guild 文本频道，通过官方 Discord Bot Gateway 集成。
...
3. Set the token for OpenAcosmi:
...
- Talk to OpenAcosmi via Discord DMs or guild channels.
...
3. Configure OpenAcosmi with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `openacosmi pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running OpenAcosmi in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents OpenAcosmi needs
...
Discord uses numeric ids everywhere; OpenAcosmi config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, OpenAcosmi treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `openacosmi doctor` and `openacosmi channels status --probe` (actionable warnings + quick audits).
...
When enabled, OpenAcosmi uses the member identity for allowlists and labels the
...
- On startup, OpenAcosmi resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror OpenAcosmi’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; OpenAcosmi enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`openacosmi gateway --force`) after confirming no other processes own the Discord session.

---
summary: "Slack 频道：Socket Mode 或 HTTP Webhook 模式"
...
> [!IMPORTANT]
> **架构状态**：Slack 频道由 **Go Gateway**（`backend/internal/channels/slack/`）实现。
...
3. Set tokens for OpenAcosmi and start the gateway.
...
7. Slash Commands → create `/openacosmi` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
OpenAcosmi can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, OpenAcosmi falls back to the user token.
...
```json
{
  "display_information": {
    "name": "OpenAcosmi",
    "description": "Slack connector for OpenAcosmi"
  },
  "features": {
    "bot_user": {
      "display_name": "OpenAcosmi",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/openacosmi",
        "description": "Send a message to OpenAcosmi",
        "should_escape": false
      }
    ]
  },
...
By default, OpenAcosmi replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, OpenAcosmi infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `openacosmi pairing approve slack <code>`.
...
- On startup, OpenAcosmi resolves channel/user names in allowlists to IDs (when tokens allow)

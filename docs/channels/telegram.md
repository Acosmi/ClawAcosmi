---
summary: "Telegram Bot API 频道：能力、配置与运维"
read_when:
...
> [!IMPORTANT]
> **架构状态**：Telegram 频道由 **Go Gateway**（`backend/internal/channels/telegram/`）实现。
> CLI 命令通过 **Rust CLI** 提供。原始 Node.js grammŸ 实现已废弃。

状态：生产就绪，支持 Bot DM + 群组。默认长轮询；Webhook 可选。
...
- 长轮询使用每聊天顺序处理；整体并发由 `agents.defaults.maxConcurrent` 封顶。
- Telegram Bot API 不支持已读回执；没有 `sendReadReceipts` 选项。

## Draft streaming

OpenAcosmi can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, OpenAcosmi retries the same message as plain text.

## Commands (native + custom)

OpenAcosmi registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; OpenAcosmi does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `openacosmi logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). OpenAcosmi can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. OpenAcosmi:
...
Private chats can include `message_thread_id` in some edge cases. OpenAcosmi keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `openacosmi logs --follow` and look for `from.id`.
...
OpenAcosmi defaults to audio files for backward compatibility.
...
OpenAcosmi defaults to video files.
...
OpenAcosmi supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, OpenAcosmi handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, OpenAcosmi caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.openacosmi/telegram/sticker-cache.json`
...
OpenAcosmi uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, OpenAcosmi:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by OpenAcosmi)
...
- Example: `openacosmi message send --channel telegram --target 123456789 --message "hi"`.
...
- `openacosmi channels status` shows a warning when config expects unmentioned group messages.
- `openacosmi channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `openacosmi logs --follow` (look for "skipping group message")
...
- 某些主机将 `api.telegram.org` 解析为 IPv6 优先。如果你的服务器没有可用的 IPv6 出口，可能导致请求卡在 IPv6 上。
- 修复：启用 IPv6 出口，或强制 `api.telegram.org` 使用 IPv4 解析（例如添加 `/etc/hosts` 条目），然后重启 Gateway。
- 快速检查：`dig +short api.telegram.org A` 和 `dig +short api.telegram.org AAAA`。
...
- `channels.telegram.proxy`: 代理 URL（SOCKS/HTTP）。

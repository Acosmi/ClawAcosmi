...
- `/usage cost` in chats: local cost summary aggregated from OpenAcosmi session logs.
- CLI: `openacosmi status --usage` prints a full per-provider breakdown.
- CLI: `openacosmi channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

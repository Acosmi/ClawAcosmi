...
read_when:
  - You want to understand what “context” means in OpenAcosmi
...
“Context” is **everything OpenAcosmi sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (OpenAcosmi-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How OpenAcosmi builds the system prompt

The system prompt is **OpenAcosmi-owned** and rebuilt each run. It includes:
...
By default, OpenAcosmi injects a fixed set of workspace files (if present):

---
summary: "基于浏览器的 Gateway 控制 UI（聊天、Node、配置）"
read_when:
  - 想要从浏览器操作 Gateway
  - 想要无需 SSH 隧道的 Tailnet 访问
title: "Control UI"
status: active
...
> [!IMPORTANT]
> **架构状态**：Control UI 由 **Go Gateway** 的 HTTP 服务器托管，通过 WebSocket 与 Gateway 通信。

Control UI 是一个小型的 **Vite + Lit** 单页应用，由 Go Gateway 服务：
...
If the page fails to load, start the Gateway first: `openacosmi gateway`.
...
you revoke it with `openacosmi devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openacosmi/openacosmi.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenAcosmi
...
OpenAcosmi **blocks** Control UI connections without device identity.
...
1. 启动 UI 开发服务器：`cd ui && npm run dev`
2. Open a URL like:

---
summary: "终端 UI（TUI）用于终端内的 Gateway 交互"
read_when:
  - 想要从终端与 Agent 聊天
  - 想要启动 TUI 并了解快捷键
title: "终端 UI (TUI)"
...
```bash
openacosmi tui --url ws://127.0.0.1:18789
```
...
- Turn delivery on:
  ...
  - or start with `openacosmi tui --deliver`
...
- Check the Gateway logs: `openacosmi logs --follow`.
- Confirm the agent can run: `openacosmi status` and `openacosmi models status`.
...
- No agents in picker: check `openacosmi agents list` and your routing config.

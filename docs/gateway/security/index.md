---
summary: "运行 AI Gateway 的安全考量与威胁模型"
read_when:
  - 添加扩展访问或自动化的功能时
title: "安全（Security）"
---
...
> [!IMPORTANT]
> **架构状态**：安全审计由 **Go Gateway**（`backend/internal/gateway/security_audit.go`）和
> CLI 命令（`cmd/openacosmi/cmd_security.go`）实现。默认端口为 `19001`。

## Quick check: `openacosmi security audit`
...
- Tighten local perms (`~/.openacosmi` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

OpenAcosmi is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenAcosmi also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.openacosmi/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openacosmi/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openacosmi/credentials/oauth.json`
...
`openacosmi security audit` warns when this setting is enabled.
...
OpenAcosmi stores session transcripts on disk under `~/.openacosmi/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openacosmi` (see the audit section below). If you need
...
OpenAcosmi can refresh the skills list mid-session:
...
- If you install plugins from npm (`openacosmi plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openacosmi/extensions/<pluginId>/` (or `$OPENACOSMI_STATE_DIR/extensions/<pluginId>/`).
  - OpenAcosmi uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
openacosmi pairing approve <channel> <code>
```
...
By default, OpenAcosmi routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenAcosmi has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openacosmi/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.openacosmi or your logs.”
...
4. **Re-run audit**
   - `openacosmi security audit --deep` and confirm the report is clean.
...
- `~/.openacosmi/openacosmi.json`: `600` (user read/write only)
- `~/.openacosmi`: `700` (user only)

`openacosmi doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENACOSMI_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_openacosmi-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENACOSMI_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openacosmi doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENACOSMI_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `OPENACOSMI_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenAcosmi
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenAcosmi verifies the identity by resolving the
...
- OpenAcosmi will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openacosmi/` (or `$OPENACOSMI_STATE_DIR/`) may contain secrets or private data:

- `openacosmi.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openacosmi status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openacosmi", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openacosmi/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openacosmi` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenAcosmi clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openacosmi gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENACOSMI_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/openacosmi/openacosmi-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openacosmi/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + OpenAcosmi version
...
Found a vulnerability in OpenAcosmi? Please report responsibly:

1. Email: [security@openacosmi.ai](mailto:security@openacosmi.ai)

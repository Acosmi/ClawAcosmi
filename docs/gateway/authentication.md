...
OpenAcosmi supports OAuth and API keys for model providers. For Anthropic
...
2. Put it on the **gateway host** (the machine running `openacosmi gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.openacosmi/.env` so the daemon can read it:
...
API keys for daemon use: `openacosmi onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.openacosmi/.env`, systemd/launchd).
...
Run `openacosmi models status` to confirm which profile is expiring. If the profile

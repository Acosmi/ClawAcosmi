---
summary: "~/.openacosmi/openacosmi.json 的所有配置选项及示例"
read_when:
  - 添加或修改配置字段
...
> [!IMPORTANT]
> **架构状态**：配置由 **Go Gateway**（`backend/internal/gateway/config_loader.go`）加载和验证。
> 沙箱配置由 **Rust** 原生实现（`cli-rust/crates/oa-sandbox/`）。默认端口为 `19001`。

OpenAcosmi reads an optional **JSON5** config from `~/.openacosmi/openacosmi.json` (comments + trailing commas allowed).

If the file is missing, OpenAcosmi uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.openacosmi/workspace`). You usually only need a config to:
...
OpenAcosmi only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `openacosmi doctor`, `openacosmi logs`, `openacosmi health`, `openacosmi status`, `openacosmi service`, `openacosmi help`).
- Run `openacosmi doctor` to see the exact issues.
- Run `openacosmi doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `openacosmi config set`. Keep a backup of `~/.openacosmi/openacosmi.json`.
...
```bash
openacosmi gateway call config.get --params '{}' # capture payload.hash
openacosmi gateway call config.apply --params '{
...
```bash
openacosmi gateway call config.get --params '{}' # capture payload.hash
openacosmi gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.openacosmi/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@openacosmi", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.openacosmi/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.openacosmi/workspace" }],
}
```
...
{ "$include": "/etc/openacosmi/base.json5" } // absolute
...
```json5
// ~/.openacosmi/clients/mueller/agents.json5
[
...
```json5
// ~/.openacosmi/clients/mueller/broadcast.json5
{
...
OpenAcosmi reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, OpenAcosmi runs your login shell and imports only the missing expected keys (never overrides).
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${OPENACOSMI_GATEWAY_TOKEN}",
    },
  },
}
```
...
OpenAcosmi stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.openacosmi/agents/<agentId>/agent/auth-profiles.json`)
...
- `~/.openacosmi/agent/*` (migrated by `openacosmi doctor` into `~/.openacosmi/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `OPENACOSMI_OAUTH_DIR`
- Agent dir (default agent root override): `OPENACOSMI_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, OpenAcosmi imports `oauth.json` entries into `auth-profiles.json`.
...
If set, OpenAcosmi derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/openacosmi/openacosmi-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/openacosmi/openacosmi.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/openacosmi/openacosmi.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.openacosmi/credentials/whatsapp/biz
          // authDir: "~/.openacosmi/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `openacosmi doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openacosmi", "openacosmi"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@openacosmi"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.openacosmi/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.openacosmi/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.openacosmi/workspace-home" },
      { id: "work", workspace: "~/.openacosmi/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `openacosmi.json`).
...
OpenAcosmi starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["openacosmi-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-openacosmi",
...
OpenAcosmi starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `openacosmi plugins install @openacosmi/mattermost` (or `./extensions/mattermost` from a git checkout).
...
OpenAcosmi starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
OpenAcosmi spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, OpenAcosmi
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/openacosmi" } },
}
```
...
When a file exceeds this limit, OpenAcosmi logs a warning and injects a truncated
...
message envelopes). If unset, OpenAcosmi uses the host timezone at runtime.
...
`[openacosmi]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[openacosmi]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[openacosmi]"`, OpenAcosmi will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, OpenAcosmi generates audio
...
OpenAcosmi also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, OpenAcosmi currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `openacosmi configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:openacosmi`: all built-in OpenAcosmi tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.openacosmi/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.openacosmi/sandboxes",
        docker: {
          ...
          apparmorProfile: "openacosmi-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openacosmi-sandbox-browser:bookworm-slim",
          containerPrefix: "openacosmi-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenAcosmi uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.openacosmi/agents/<agentId>/agent/models.json` or by defining the same schema inside your
OpenAcosmi config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, OpenAcosmi writes/merges a `models.json` into
`~/.openacosmi/agents/<agentId>/agent/` on startup:
...
### OpenAcosmi Zen (multi-model proxy)

OpenAcosmi Zen is a multi-model gateway with per-model endpoints. OpenAcosmi uses
the built-in `openacosmi` provider from pi-ai; set `OPENACOSMI_API_KEY` (or
`OPENACOSMI_ZEN_API_KEY`) from [https://openacosmi.com/auth](https://openacosmi.com/auth).

Notes:

- Model refs use `openacosmi/<modelId>` (example: `openacosmi/claude-opus-4-6`).
- If you enable an allowlist via `agents.defaults.models`, add each model you plan to use.
- Shortcut: `openacosmi onboard --auth-choice openacosmi-zen`.

```json5
{
  agents: {
    defaults: {
      model: { primary: "openacosmi/claude-opus-4-6" },
      models: { "openacosmi/claude-opus-4-6": { alias: "Opus" } },
    },
  },
}
```
...
Shortcut: `openacosmi onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `openacosmi onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `openacosmi onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `openacosmi onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `openacosmi onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `openacosmi onboard --auth-choice minimax-api`.
...
- Override the agent config root with `OPENACOSMI_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.openacosmi/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.openacosmi/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.openacosmi/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `direct`, `group`, and `thread`. Legacy `dm` key is accepted as an alias for `direct`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, OpenAcosmi stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.openacosmi/skills` (workspace skills
...
from `~/.openacosmi/extensions`, `<workspace>/.openacosmi/extensions`, plus any
...
### `browser` (openacosmi-managed browser)

OpenAcosmi can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for openacosmi and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (OpenAcosmi.app menubar, or `openacosmi gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      openacosmi: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/openacosmi" }
...
- Examples: `"/ui"`, `"/openacosmi"`, `"/apps/openacosmi"`.
...
- When a connection comes from one of these IPs, OpenAcosmi uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `openacosmi gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `OPENACOSMI_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `OPENACOSMI_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. OpenAcosmi
...
- OpenAcosmi.app watches `~/.openacosmi/openacosmi.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.openacosmi/openacosmi.json` (or `OPENACOSMI_CONFIG_PATH`) and applies changes automatically.
...
- `~/.openacosmi/openacosmi.json` (or `OPENACOSMI_CONFIG_PATH`)
...
- `OPENACOSMI_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `openacosmi --dev …` → uses `~/.openacosmi-dev` + shifts ports from base `19001`
- `openacosmi --profile <name> …` → uses `~/.openacosmi-<name>` (port via config/env/flags)
...
```bash
OPENACOSMI_CONFIG_PATH=~/.openacosmi/a.json \
OPENACOSMI_STATE_DIR=~/.openacosmi-a \
openacosmi gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.openacosmi/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail helper config (used by `openacosmi webhooks gmail setup` / `run`):
...
- Set `OPENACOSMI_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, OpenAcosmi defaults `serve.path` to `/` so
...
Default root: `~/.openacosmi/workspace/canvas`  
Default port: `18793` (chosen to avoid the openacosmi browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__openacosmi__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__openacosmi__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.openacosmi/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `OPENACOSMI_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.openacosmi/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.openacosmi/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.openacosmi/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_openacosmi-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `openacosmi` (advertises `openacosmi.local`). Override with `OPENACOSMI_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_openacosmi-gw._tcp` under `~/.openacosmi/dns/` using the configured discovery domain (example: `openacosmi.internal.`).

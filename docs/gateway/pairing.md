...
openacosmi nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.openacosmi`):

- `~/.openacosmi/nodes/paired.json`
- `~/.openacosmi/nodes/pending.json`

If you override `OPENACOSMI_STATE_DIR`, the `nodes/` folder moves with it.

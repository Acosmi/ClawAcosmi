...
- `openacosmi gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `openacosmi doctor` reports no blocking config/service issues.
- `openacosmi channels status --probe` shows connected/ready channels.
...
openacosmi browser start --browser-profile openacosmi
...
```bash
openacosmi gateway status
openacosmi config get gateway.mode
openacosmi config get gateway.remote.url
openacosmi config get gateway.auth.mode
```
...
```bash
openacosmi config get gateway.bind
openacosmi config get gateway.auth.token
openacosmi gateway status
openacosmi logs --follow
```

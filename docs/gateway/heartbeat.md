...
- During heartbeat runs, OpenAcosmi treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openacosmi sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenAcosmi skips the heartbeat run entirely (no model call).
...
```bash
openacosmi system event --text "Check for urgent follow-ups" --mode now
```

...
- Replaces the legacy `gateway` command. CLI entry point: `openacosmi gateway`.
...
openacosmi gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.openacosmi/openacosmi.json` (or `OPENACOSMI_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openacosmi__/canvas/` from `~/.openacosmi/workspace/canvas`. Disable with `canvasHost.enabled=false` or `OPENACOSMI_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `OPENACOSMI_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `OPENACOSMI_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.openacosmi.*` may still exist)
...
openacosmi --dev gateway --allow-unconfigured
...
- Base port = `gateway.port` (or `OPENACOSMI_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `OPENACOSMI_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `OPENACOSMI_CONFIG_PATH`
- unique `OPENACOSMI_STATE_DIR`
...
```bash
OPENACOSMI_CONFIG_PATH=~/.openacosmi/a.json OPENACOSMI_STATE_DIR=~/.openacosmi-a openacosmi gateway --port 19001
OPENACOSMI_CONFIG_PATH=~/.openacosmi/b.json OPENACOSMI_STATE_DIR=~/.openacosmi-b openacosmi gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `openacosmi health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `openacosmi`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `openacosmi gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.openacosmi.*` is cleaned up).
  - `openacosmi doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are OpenAcosmi profile services.
  ...
  - Cleanup: `openacosmi gateway uninstall` (current service) and `openacosmi doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `openacosmi gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- OpenAcosmi.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.openacosmi.*` labels still unload cleanly).
- To stop it cleanly, use `openacosmi gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `openacosmi gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `openacosmi gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

OpenAcosmi installs a **systemd user service** by default on Linux/WSL2. We
...
`openacosmi gateway install` writes the user unit. `openacosmi doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/openacosmi-gateway[-<profile>].service`:

```
[Unit]
Description=OpenAcosmi Gateway (profile: <profile>, v<version>)
...
Create `/etc/systemd/system/openacosmi-gateway[-<profile>].service` (copy the unit above,
...
- `openacosmi gateway health|status` — request health/status over the Gateway WS.
- `openacosmi message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `openacosmi agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `openacosmi gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `openacosmi gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `openacosmi gateway` and the legacy TCP control port.

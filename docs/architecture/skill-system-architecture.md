...
- `tools:` — **工具绑定**（可选），逗号分隔的工具名列表。声明后，技能描述会自动注入到对应工具的 Description 末尾，LLM 读工具定义时即获得技能指引，无需额外搜索。详见 [工具-技能绑定架构](tool-skill-binding-architecture.md)。
...
| 文件 | 职责 |
|------|------|
| `skills/workspace_skills.go` | `LoadSkillEntries` 全量扫描 + `ResolveDocsSkillsDir` 自动发现 + `deduplicateEntries` 去重 + `ResolveToolSkillBindings` 工具绑定索引 |
| `skills/frontmatter.go` | Frontmatter 解析 + OpenAcosmi 元数据（含 `Tools` 字段） + 调用策略 |
| `skills/bundled_dir.go` | Bundled skills 目录定位（多策略） |
| `runner/attempt_runner.go` | `buildSystemPrompt` 注入 `SkillsPrompt` + 构建 `toolBindings`；`buildToolDefinitions` 注入绑定描述 |
...
r.toolBindings = skills.ResolveToolSkillBindings(entries) // 复用 entries 构建绑定
...
### 3.5 工具-技能绑定（Tool-Skill Binding）

技能可通过 frontmatter `tools:` 字段声明绑定的工具名。绑定后，技能描述自动追加到工具 Definition 的 Description 末尾，LLM 无需搜索即获得技能指引。

**数据流**:

```
SKILL.md frontmatter tools: bash
         │
         ▼
loadSkillsFromDir() → SkillEntry.Metadata.Tools = ["bash"]
         │
         ▼
ResolveToolSkillBindings(entries) → map["bash"] = "Exec tool usage..."
         │
         ▼
buildToolDefinitions() → tools[i].Description += " [Skill: Exec tool usage...]"
         │
         ▼
LLM 读工具定义时自动获得技能指引
```
...
| 工具名 | 绑定技能 | 技能描述 |
|--------|---------|---------|
| `bash` | exec | Exec tool usage, stdin modes, and TTY support |
| `browser` | browser | Integrated browser control service + action commands |
| `send_media` | send-media | 发送文件/媒体到远程频道 |
| `web_search` | web | Web search + fetch tools |
| `spawn_coder_agent` | coder | Programming sub-agent |
| `spawn_argus_agent` | argus-visual | Argus visual sub-agent |
| `search_skills` | skills | 技能系统 |
| `lookup_skill` | skills | 技能系统（同上） |
| `write_file` | apply-patch | Apply multi-file patches |

**新增绑定方法**: 见 [工具-技能绑定架构](tool-skill-binding-architecture.md) §三。
...
| 工具绑定数 | 10（覆盖 9 个工具名，来自 8 个 SKILL.md） |
| 核心函数 | `LoadSkillEntries` / `ResolveToolSkillBindings` / `buildSystemPrompt` / `buildToolDefinitions` |
| 测试 | 9 pass（含 4 个绑定相关测试） |

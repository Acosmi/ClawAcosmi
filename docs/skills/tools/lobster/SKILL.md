---
name: lobster
description: "Typed workflow runtime for OpenAcosmi with resumable approval gates."
---

# Lobster

Lobster is a workflow shell that lets OpenAcosmi run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: OpenAcosmi runs one Lobster tool call and gets a structured result.
...
OpenAcosmi launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In OpenAcosmi tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the OpenAcosmi Gateway (see the [Lobster repo](https://github.com/openacosmi/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), OpenAcosmi keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls OpenAcosmi tools that do.

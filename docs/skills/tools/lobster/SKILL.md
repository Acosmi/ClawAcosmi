---
name: lobster
description: "Typed workflow runtime for Claw Acismi with resumable approval gates."
---

# Lobster

Lobster is a workflow shell that lets Claw Acismi run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Claw Acismi runs one Lobster tool call and gets a structured result.
...
Claw Acismi launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Claw Acismi tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Claw Acismi Gateway (see the [Lobster repo](https://github.com/openacosmi/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Claw Acismi keeps core tools enabled. To restrict core
...
→ openacosmi calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ openacosmi calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Claw Acismi tools that do.

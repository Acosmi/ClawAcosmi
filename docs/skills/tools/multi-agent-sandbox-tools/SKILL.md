...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openacosmi sandbox explain`.
...
- `group:nodes`: `nodes`
- `group:openacosmi`: all built-in Claw Acismi tools (excludes provider plugins)
...
Legacy `agent.*` configs are migrated by `openacosmi doctor`; prefer `agents.defaults` + `agents.list` going forward.

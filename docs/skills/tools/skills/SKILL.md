...
# 技能系统（创宇太虚（Claw Acismi））

创宇太虚（Claw Acismi） 使用兼容 AgentSkills 格式的技能目录向智能体注入额外能力。每个技能是一个包含 `SKILL.md` 的目录，文件包含 YAML frontmatter 元数据和 Markdown 指令。系统启动时按优先级加载技能，依据环境、配置和可执行文件可用性过滤。
...
创宇太虚（Claw Acismi） 内置技能商店客户端，连接 nexus-v4 MCP Server，支持浏览、拉取、刷新、链接技能：
...
创宇太虚（Claw Acismi） 在**会话开始时**对符合条件的技能拍摄快照，在同一会话的后续轮次中复用该列表。技能或配置的变更在下一个新会话生效。

## Token 影响（技能索引）

技能符合条件时，创宇太虚（Claw Acismi） 将紧凑索引注入 system prompt：

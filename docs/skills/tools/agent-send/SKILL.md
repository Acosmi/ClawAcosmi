---
name: agent-send
description: "Direct `openacosmi agent` CLI runs (with optional delivery)"
---

# `openacosmi agent` (direct agent runs)

`openacosmi agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `openacosmi message --target`).
...
openacosmi agent --to +15555550123 --message "Trace logs" --verbose on --json

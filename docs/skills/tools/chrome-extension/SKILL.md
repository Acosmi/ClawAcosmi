...
description: "Chrome extension: let OpenAcosmi drive your existing Chrome tab"
---

# Chrome extension (browser relay)

The OpenAcosmi Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openacosmi-managed Chrome profile.
...
OpenAcosmi then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenAcosmi release (npm package) as static files. There is no separate “build” step.

After upgrading OpenAcosmi:

- Re-run `openacosmi browser extension install` to refresh the installed files under your OpenAcosmi state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

OpenAcosmi ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want OpenAcosmi to control.
...
- `ON`: attached; OpenAcosmi can drive that tab.
...
Debugging: `openacosmi sandbox explain`
...
`openacosmi browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openacosmi browser extension install` first to copy the extension to a stable location under your OpenAcosmi state directory.

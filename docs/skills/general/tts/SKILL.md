...
Claw Acismi can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Claw Acismi can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Claw Acismi defaults
...
- If `provider` is **unset**, Claw Acismi prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Claw Acismi retries with MP3.

OpenAI/ElevenLabs formats are fixed; Telegram expects Opus for voice-note UX.

## Auto-TTS behavior

When enabled, Claw Acismi:
...
Discord note: `/tts` is a built-in Discord command, so Claw Acismi registers
...
/tts limit 2000
/tts summary off
/tts audio Hello from Claw Acismi
```

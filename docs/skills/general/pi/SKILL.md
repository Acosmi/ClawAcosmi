---
name: pi
description: "Pi Integration Architecture"
---

# Pi Integration Architecture

This document describes how OpenAcosmi integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

OpenAcosmi uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, OpenAcosmi directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in OpenAcosmi's local TUI mode)                                             |
...
├── pi-tools.ts                    # createOpenAcosmiCodingTools()
...
├── openacosmi-tools.ts              # OpenAcosmi-specific tools
...
2. **Custom Replacements**: OpenAcosmi replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **OpenAcosmi Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures OpenAcosmi's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, OpenAcosmi CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
OpenAcosmi wraps this with `guardSessionManager()` for tool result safety.
...
OpenAcosmi maintains an auth profile store with multiple API keys per provider:
...
OpenAcosmi loads custom pi extensions for specialized behavior:
...
OpenAcosmi also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | OpenAcosmi Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom OpenAcosmi tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.openacosmi/agents/<agentId>/sessions/` (or `$OPENACOSMI_STATE_DIR/agents/<agentId>/sessions/`) |
...
- `src/agents/pi-tools.create-openacosmi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-b.test.ts`
- `src/agents/pi-tools.create-openacosmi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-d.test.ts`
- `src/agents/pi-tools.create-openacosmi-coding-tools.adds-claude-style-aliases-schemas-without-dropping-f.test.ts`
- `src/agents/pi-tools.create-openacosmi-coding-tools.adds-claude-style-aliases-schemas-without-dropping.test.ts`

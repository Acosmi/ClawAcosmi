---
name: ollama
description: "Run Claw Acismi with Ollama (local LLM runtime)"
---

# Ollama

Ollama is a local LLM runtime that makes it easy to run open-source models on your machine. Claw Acismi integrates with Ollama's OpenAI-compatible API and can **auto-discover tool-capable models** when you opt in with `OLLAMA_API_KEY` (or an auth profile) and do not define an explicit `models.providers.ollama` entry.
...
3. Enable Ollama for Claw Acismi (any value works; Ollama doesn't require a real key):
...
When you set `OLLAMA_API_KEY` (or an auth profile) and **do not** define `models.providers.ollama`, Claw Acismi discovers models from the local Ollama instance at `http://127.0.0.1:11434`:
...
If `OLLAMA_API_KEY` is set, you can omit `apiKey` in the provider entry and Claw Acismi will fill it for availability checks.
...
Claw Acismi marks models as reasoning-capable when Ollama reports `thinking` in `/api/show`:
...
For auto-discovered models, Claw Acismi uses the context window reported by Ollama when available, otherwise it defaults to `8192`. You can override `contextWindow` and `maxTokens` in explicit provider config.
...
Claw Acismi only auto-discovers models that report tool support. If your model isn't listed, either:
...
If you see garbled responses containing tool names (like `sessions_send`, `memory_get`) or fragmented text when using Ollama models, this is due to an upstream SDK issue with streaming responses. **This is fixed by default** in the latest Claw Acismi version by disabling streaming for Ollama models.

---
name: bedrock
description: "Use Amazon Bedrock (Converse API) models with Claw Acismi"
---

# Amazon Bedrock

Claw Acismi can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, Claw Acismi can automatically discover Bedrock
...
When running Claw Acismi on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, Claw Acismi's credential detection currently only checks for environment
...
- Claw Acismi surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,

---
summary: "Agent 工具面板（浏览器、canvas、node、消息、定时任务）"
read_when:
  - 添加或修改 Agent 工具
  - 调整工具配置
title: "工具"
...
> [!IMPORTANT]
> **架构状态**：工具由 **Go Gateway** 内部实现（`backend/internal/agents/tools/`），
> Agent 通过 WebSocket RPC 调用工具。

OpenAcosmi 暴露**一级 Agent 工具**用于浏览器、canvas、node 和定时任务。
这些工具是类型化的，无需 shell 调用，Agent 应直接依赖它们。

---
summary: "Agent tool surface for OpenAcosmi (browser, canvas, nodes, message, cron) replacing legacy `openacosmi-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `openacosmi-*` skills
title: "Tools"
---

# Tools (OpenAcosmi)

OpenAcosmi exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `openacosmi-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `openacosmi.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, OpenAcosmi logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`openacosmi node run`).
...
- Requires a Brave API key (recommended: `openacosmi configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated OpenAcosmi-managed browser.
...
- Quick smoke: `openacosmi nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `openacosmi gateway` restart in-place)

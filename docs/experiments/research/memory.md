...
read_when:
  - Designing workspace memory (~/.openacosmi/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenAcosmi integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openacosmi/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openacosmi/workspace` as canonical human-readable memory.
...
Recommendation: **deep integration in OpenAcosmi**, but keep a separable core library.

### Why integrate into OpenAcosmi?

- OpenAcosmi already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openacosmi memory recall "…" --k 25 --since 30d`
  - `openacosmi memory reflect --since 7d`
...
Pragmatic take for `~/.openacosmi/workspace`:

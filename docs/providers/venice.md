---
summary: "Use Venice AI privacy-focused models in OpenAcosmi"
read_when:
  - You want privacy-focused inference in OpenAcosmi
...
After setup, OpenAcosmi shows all available Venice models. Pick based on your needs:
...
## Configure via `openacosmi configure`

1. Run `openacosmi configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenAcosmi automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
The Venice model catalog updates dynamically. Run `openacosmi models list` to see currently available models. Some models may be temporarily offline.

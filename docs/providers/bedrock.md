---
summary: "Use Amazon Bedrock (Converse API) models with OpenAcosmi"
read_when:
  - You want to use Amazon Bedrock models with OpenAcosmi
...
OpenAcosmi can use **Amazon Bedrock** models via pi‑ai’s **Bedrock Converse**
...
If AWS credentials are detected, OpenAcosmi can automatically discover Bedrock
...
When running OpenAcosmi on an EC2 instance with an IAM role attached, the AWS SDK
will automatically use the instance metadata service (IMDS) for authentication.
However, OpenAcosmi's credential detection currently only checks for environment
...
- OpenAcosmi surfaces the credential source in this order: `AWS_BEARER_TOKEN_BEDROCK`,

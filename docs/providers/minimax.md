---
summary: "Use MiniMax M2.1 in OpenAcosmi"
read_when:
  - You want MiniMax models in OpenAcosmi
...
```bash
openacosmi plugins enable minimax-portal-auth  # skip if already loaded.
openacosmi gateway restart  # restart if gateway is already running
...
See [MiniMax OAuth plugin README](https://github.com/openacosmi/openacosmi/tree/main/extensions/minimax-portal-auth) for details.
...
- Use `openacosmi models list` and `openacosmi models set minimax/MiniMax-M2.1` to switch.
...
- Running `openacosmi configure` and selecting **MiniMax M2.1**, or

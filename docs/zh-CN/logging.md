...
你可以在 `~/.openacosmi/openacosmi.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openacosmi/openacosmi.json` 的 `logging` 下。
...
- OpenAcosmi 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openacosmi plugins enable diagnostics-otel` 启用插件。
...
- **Gateway 网关无法访问？** 先运行 `openacosmi doctor`。

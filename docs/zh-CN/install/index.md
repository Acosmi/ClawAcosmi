...
read_when:
  - 安装 OpenAcosmi
  - 你想从 GitHub 安装
summary: 安装 OpenAcosmi（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
通过 npm 全局安装 `openacosmi` 并运行新手引导。
...
pnpm approve-builds -g                # 批准 openacosmi、node-llama-cpp、sharp 等
pnpm add -g openacosmi@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm openacosmi ...` 运行仓库命令。
...
- 运行新手引导：`openacosmi onboard --install-daemon`
- 快速检查：`openacosmi doctor`
- 检查 Gateway 网关健康状态：`openacosmi status` + `openacosmi health`
- 打开仪表板：`openacosmi dashboard`
...
```bash
# 显式 npm
curl -fsSL https://openacosmi.ai/install.sh | bash -s -- --install-method npm
...
- `OPENACOSMI_GIT_DIR=...`
- `OPENACOSMI_GIT_UPDATE=0|1`
- `OPENACOSMI_NO_PROMPT=1`
- `OPENACOSMI_DRY_RUN=1`
- `OPENACOSMI_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `openacosmi`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `openacosmi`）。

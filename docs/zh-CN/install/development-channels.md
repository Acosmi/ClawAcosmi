...
当你使用 `--channel` **显式**切换渠道时，OpenAcosmi 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/openacosmi`，可通过 `OPENACOSMI_GIT_DIR` 覆盖），
...
当你使用 `openacosmi update` 切换渠道时，OpenAcosmi 还会同步插件来源：

...
summary: 设置指南：在保持最新的同时保持你的 OpenAcosmi 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.openacosmi/workspace`（工作区）+ `~/.openacosmi/openacosmi.json`（配置）。
...
- **工作区：** `~/.openacosmi/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm openacosmi setup` 运行它。
...
- 运行 `openacosmi setup`，然后 `openacosmi channels login`，然后手动启动 Gateway 网关（`openacosmi gateway`）。
...
- **模型认证配置文件**：`~/.openacosmi/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.openacosmi/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.openacosmi/workspace` 和 `~/.openacosmi/` 保持为"你的东西"；不要将个人提示/配置放入 `openacosmi` 仓库。

...
- `agents.defaults.models` 是 OpenAcosmi 可使用的模型白名单/目录（加上别名）。
...
提供商配置示例（包括 OpenAcosmi Zen）在 [/gateway/configuration](/gateway/configuration#openacosmi-zen-multi-model-proxy)。

## "Model is not allowed"（以及为什么回复停止）

如果设置了 `agents.defaults.models`，它将成为 `/model` 和会话覆盖的**白名单**。当用户选择不在该白名单中的模型时，OpenAcosmi 返回：
...
- 如果省略提供商，OpenAcosmi 将输入视为别名或**默认提供商**的模型（仅在模型 ID 中没有 `/` 时有效）。
...
openacosmi models set <provider/model>
openacosmi models set-image <provider/model>

openacosmi models aliases list
openacosmi models aliases add <alias> <provider/model>
openacosmi models aliases remove <alias>

openacosmi models fallbacks list
openacosmi models fallbacks add <provider/model>
openacosmi models fallbacks remove <provider/model>
openacosmi models fallbacks clear

openacosmi models image-fallbacks list
openacosmi models image-fallbacks add <provider/model>
openacosmi models image-fallbacks remove <provider/model>
openacosmi models image-fallbacks clear
```

`openacosmi models`（无子命令）是 `models status` 的快捷方式。
...
`openacosmi models scan` 检查 OpenRouter 的**免费模型目录**，并可选择性地探测模型的工具和图像支持。
...
`models.providers` 中的自定义提供商会写入智能体目录下的 `models.json`（默认 `~/.openacosmi/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件默认会被合并。

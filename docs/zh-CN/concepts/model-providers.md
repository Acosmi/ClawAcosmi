...
- 模型引用使用 `provider/model` 格式（例如：`openacosmi/claude-opus-4-6`）。
- 如果设置了 `agents.defaults.models`，它将成为允许列表。
- CLI 辅助工具：`openacosmi onboard`、`openacosmi models list`、`openacosmi models set <provider/model>`。

## 内置提供商（pi-ai 目录）

OpenAcosmi 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`openacosmi onboard --auth-choice token`（粘贴 setup-token）或 `openacosmi models auth paste-token --provider anthropic`
...
- CLI：`openacosmi onboard --auth-choice openai-codex` 或 `openacosmi models auth login --provider openai-codex`
...
- 提供商：`openacosmi`
- 认证：`OPENACOSMI_API_KEY`（或 `OPENACOSMI_ZEN_API_KEY`）
- 示例模型：`openacosmi/claude-opus-4-6`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`openacosmi plugins enable google-antigravity-auth`
  - 登录：`openacosmi models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`openacosmi plugins enable google-gemini-cli-auth`
  - 登录：`openacosmi models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `openacosmi.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
